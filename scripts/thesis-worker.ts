...
export async function cycle(statusPath = resolve(".data/thesis-worker-status.json")) {
  let previous: EvidenceStatus = { lastSuccess: null, lastError: null, unresolvedChecks: [] };
  try {
    const saved = JSON.parse(readFileSync(statusPath, "utf8"));
    previous = {
      lastSuccess: typeof saved.lastSuccess === "string" ? saved.lastSuccess : null,
      lastError: typeof saved.lastError === "string" ? saved.lastError : null,
      unresolvedChecks: Array.isArray(saved.unresolvedChecks) ? saved.unresolvedChecks.filter((id: unknown): id is string => typeof id === "string") : [],
    };
  } catch (error) {
  ...
  try {
    for (const item of store.due()) {
      try {
        ...
        if (latest?.error || !latest?.sample) { failed++; unresolved.add(item.id); }
        else { checked++; unresolved.delete(item.id); }
      } catch { failed++; unresolved.add(item.id); }
    }
  } finally { store.close(); }
  ...
  const status = {
    ...
    lastSuccess: checked > 0 && unresolved.size === 0 ? at : previous.lastSuccess,
    lastError: unresolved.size ? `${unresolved.size} scheduled thesis source(s) remain unverified after failure.`
      : checked > 0 ? null : previous.lastError,
...
async function main() {
  ...
  if (process.argv.includes("--watch"))
    setInterval(() => void cycle().catch(() => console.error("Thesis worker cycle failed.")), 60000);
}
